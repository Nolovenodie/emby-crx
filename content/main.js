class Home {
	...
	static async getItem(itemId) {
		// 双缓存 优先使用 WebStorage // Double cache, give priority to WebStorage
	...
	static async initBanner() {
		...
		data.Items.forEach(async (item) => {
			...
			<div class="misty-banner-item" id="${detail.Id}">
				...
				<div class="misty-banner-info padded-left padded-right">
					...
					<div><button onclick="appRouter.showItem('${detail.Id}')">MORE INFO</button></div>
				</div>
			</div>
			`,
				logoHtml = `
		...
		// 只判断第一张海报加载完毕, 优化加载速度 // Only check if the first poster is loaded, to optimize loading speed
		...
		// 判断section0加载完毕 // Determine if section0 is loaded
		await new Promise((resolve, reject) => {
		...
		// 置入场动画 // Insert the animation
		let delay = 80; // 动媒体库画间隔 // Animation library drawing interval
		let id = $(".misty-banner-item").eq(0).addClass("active").attr("id"); // Initial information animation
		$(`.misty-banner-logo[id=${id}]`).addClass("active");

		await CommonUtils.sleep(200); // 间隔动画 // Interval animation
		$(".section0 > div").addClass("misty-banner-library-overflow"); // 关闭overflow 防止媒体库动画溢出 // Turn off overflow to prevent media library animation from overflowing
		$(".misty-banner .card").each((i, dom) => setTimeout(() => $(dom).addClass("misty-banner-library-show"), i * delay)); // 媒体库动画 // Media library animation
		await CommonUtils.sleep(delay * 8 + 1000); // 等待媒体库动画完毕 // Wait for the media library animation to complete
		$(".section0 > div").removeClass("misty-banner-library-overflow"); // 开启overflow 防止无法滚动 // Turn on overflow to prevent scrolling

		// 滚屏逻辑 // Scrolling logic
		var index = 0;
	...
	/* 初始事件 */ /* Initial event */
	static initEvent() {
		// 通过注入方式, 方可调用appRouter函数, 以解决Content-Script window对象不同步问题 // Only by injecting can the appRouter function be called to solve the problem of Content-Script window object being out of sync
		const script = `
		// 挂载appRouter
